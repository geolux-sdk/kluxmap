...
### 7. `filtering()`이 루프 변수 `df`를 루프 밖에서 사용
...
### 8. `_apply_calibration`이 조기 종료 시 `None` 반환
...
### 11. 공유 가변 config dict 참조
- 위치: `kriging_dialog.py:341`
- `self.filters = config.get("kriging", {})`가 라이브 dict를 반환해 여러 다이얼로그가 상태를 공유/덮어쓸 수 있음.
- 수정: `dict(config.get("kriging", {}))`로 복사.

### 12. 콜백 / close 정리 시 `self.im` / `parent()` 가정
