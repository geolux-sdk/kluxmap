...
### 10. KDTree에 NaN 좌표 유입 가능 — ✅ 수정 완료 (2026-06-24)
...
- **상태: 해결됨.** `median2d_on`에서 values/E/N을 `np.asarray(..., float)`로 강제하고, `np.isfinite(E) & np.isfinite(N)` 마스크로 **유효 좌표만으로 cKDTree를 구성**. 이웃 인덱스는 `valid_idx`로 원본에 재매핑하고, NaN 좌표 점은 원래 값으로 폴백. 유효 좌표가 하나도 없으면 입력을 그대로 반환.
...
- **죽은 코드** — ✅ 수정 완료 (2026-06-24, 주석 라인 삭제):
  - `Data_Parse.py` 읽고 버려지던 CSV 로드 주석(`# df_list = [pd.read_csv(...)]`) 삭제.
  - `FlightPlotWidget.py` 값 버리던 `# config.get("filters", {})` 주석 라인 삭제.
  - ~~`parse_input_data` vs `parse_input_data_v2025` 중복~~ — **오판정 정정**: 내용이 비슷하나 처리 형식이 다른 별개 로직으로, 중복 아님.
- **오타** — ✅ 수정 완료 (2026-06-24): `CalibrationFlightWidget.py:690` `_main_direiion_str` → `_main_direction_str`, `myWidgets.py` `browse_multidirectiorys` → `browse_multidirectorys`(정의·로그 문자열 모두). 두 식별자 모두 외부 참조 없음을 확인 후 변경.
- **deprecated API** — ✅ 수정 완료 (2026-06-24):
  - `datetime.utcnow()`(`LinePlotWidget.py`) → `pd.Timestamp.now(tz="UTC").tz_localize(None)`로 교체(다른 분기와 동일하게 naive 유지). 미사용이 된 `from datetime import datetime` import도 제거.
  - `QDialog.exec_()`(`myConvertDlg.py`) → `exec()`.
  - `ContourSet.collections`(`kriging_dialog.py`) → matplotlib 3.10에서 `.collections` 제거로 `hasattr` 가드가 False가 되어 컨투어가 정리되지 않던 누수. `ContourSet`이 곧 `Collection`이므로 `self.contour_set.remove()`를 직접 호출하도록 변경.
- **pyplot 전역 등록 figure 누수** — ✅ 수정 완료 (2026-06-24): LinePlotWidget(`:171`,`:184`)·FlightPlotWidget(`:517`,`:537`)·kriging_dialog(`:507`)의 `plt.subplots`를 CalibrationFlightWidget과 동일하게 `Figure(...)` + `add_subplot(111)` 직접 생성으로 전환해 pyplot 전역 레지스트리 등록을 제거(위젯 재생성 시 figure 누수 방지). FlightPlotWidget의 `plt.tight_layout(...)`은 임베드 figure를 대상으로 `self.fig.tight_layout(...)`로 변경. kriging_dialog의 임시 export figure(`:808`,`:922`)는 `plt.close`로 이미 정리되므로 그대로 둠.
